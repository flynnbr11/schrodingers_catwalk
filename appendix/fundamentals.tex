...
The inner product is often called the \textir{scalar product}, which is in general complex.

\end{itemize}


\section{Postulates of \glsentrylong{qm}}
...
\begin{enumerate}
\item Every moving particle in a conservative force field has an associated wave-function, $\ket{\psi}$. From this wave-function, it is possible to determine all physical information about the system. 
\item All particles have physical properties called observables (denoted $q$). In order to determine a value, $q$, for a particular observable, there is an associated \emph{operator} $\hat{Q}$, which, when acting on the particles wavefunction, yields the value times the wavefunction. The observable $q$ is then the eigenvalue of the operator $\hat{Q}$.
    \begin{equation}
    \hat{Q} \ket{\psi} = q | \psi \rangle
    \end{equation}
\item Any such operator  $\hat{Q}$ is Hermitian
    \begin{equation}
    \hat{Q}^\dag = \hat{Q}
    \end{equation}
\item The set of eigenfunctions for any operator $\hat{Q}$ forms a complete set of linearly independent functions.
\item For a system with wavefunction $ | \psi \rangle$, the expectation value of an observable $q$  with respect to an operator $\hat{Q}$ is  denoted by $\la q \ra$ and is given by 
    \begin{equation}
    \la q \ra = \langle \psi | \hat{Q} | \psi \rangle
    \end{equation}
\item The time evolution of $ | \psi \rangle$ is given by the time dependent \emph{Schrodinger Equation}
    \begin{equation}
...
\begin{equation}
    \begin{split}
        ...
        \\ 
        \Rightarrow \psi &= \frac{1}{\sqrt{2}} \ket{\leftrightarrow} \pm \frac{1}{\sqrt{2}} \ket{\updownarrow} 
        \\
        \Rightarrow \psi &= \frac{1}{\sqrt{2}} \ket{0} \pm \frac{1}{\sqrt{2}} \ket{1}
    \end{split}
\end{equation} 
...
That is, the system is given by a linear combination of the four basis vectors
\begin{equation}
...
\section{Unitary Transformations} \label{unitary}
...
\begin{example}
    ...
    \begin{equation}
        \braket{11|10} = \braket{00|10} =  \braket{01|10} = \braket{11|01} = \braket{00|01}  = \braket{10|01} = 0 \ \textrm{and} \ \braket{10|10} = \braket{01|01} = 1 
    \end{equation}
...
As we move to systems of larger size, standard matrix multiplication will become unfeasible. 
Then it is obvious that Dirac's bra/ket notation can be considered best practice for the scope of this subject and quantum mechanics in general. 