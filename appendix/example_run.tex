...
(i) install \ttt{redis};
(ii) create a virtual Python environment for installing \gls{qmla} dependencies without damaging other parts of 
...
Here, we recommend running a number of \glspl{instance} with very few resources so that the test finishes quickly\footnote{This run will take about ten minutes}. 
...
The champion model from each \gls{instance} can attempt to reproduce system dynamics: 
...
\begin{figure}[H]
    ...
    \caption[Run plots]{
        ...
        \textbf{(b)} \ttt{champion\_models/params\_params\_pauliSet\_1J2\_zJz\_d4+pauliSet\_2J3\_zJz\_d4+pauliSet\_3J4\_zJz\_d4}: 
            parameter estimation progression for the true model, only for the \glspl{instance} where it was deemed champion. 
