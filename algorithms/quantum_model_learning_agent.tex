...
Given a target quantum system, \gls{q}, described by some \emph{true} \gls{hamiltonian} model, $\ho$, 
...
\begin{algorithm}
    \caption{Quantum \gls{hamiltonian} Learning}
    \label{alg:qhl}
    \DontPrintSemicolon
    ...
    \KwIn{\ttt{$\Lambda(\pra)$} \tcp*[1]{Heuristic algorithm which designs \glspl{experiment}}}
    ...
    Sample \gls{nprt} times from $\pra \gets \mathcal{P}$ \tcp*[1]{\glspl{particle}}\;
    \For{
        $p \in \mathcal{P}$ 
    }{
    ...
    \For{$ e \in  \{1 \rightarrow \Ne\}$ }{ 
        ...
        \For{
            ...
            Retrieve \gls{particle} $p \gets \al_p$
...
\begin{algorithm}
    ...
    \KwIn{ $\expset_j, \expset_k$ \tcp*[1]{\glspl{experiment} on which $\hat{H}_j^{\prime}$ and $\hat{H}_k^{\prime}$ were trained during QHL.}}
    ...
    \For{$\hat{H}_i^{\prime} \in  \{\hat{H}_j^{\prime}, \hat{H}_k^{\prime}$ \} }{
        ...
        \For{$e \in \expset$}{
            $e \gets t, \ket{\psi}$ \tcp*[1]{assign time and \gls{probe} from \gls{experiment} control set}\;
