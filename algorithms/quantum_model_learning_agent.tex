...
Each term is associated with a multiplicative scalar, which may be referred to as that term's \emph{parameter}: 
...
\glspl{bf} are known to be statistically significative of the stronger model 
    ...
    while favouring models of low cardinality, thereby suppressing overfitting models. 
\par 
...
Therefore $\lvert B_{ij} \rvert$ is the strength of the statistical evidence
...
\footnotetext{We will verify this claim in \cref{sec:bf_by_f_score}, in the context of real examples.}
\par 

A final option is to design a set of \emph{evaluation} \glspl{experiment}, $\expset_v$, 
...
\begin{description}
    ...
    The performance of candidates in $\mu$ are ranked relative to each other, 
        ...
        Consolidation can rely on any statistical test, with \glspl{bf} providing a robust platform to distinguish 
...
\begin{figure}[t!]
    ...
    \caption[Quantum Model Learning Agent overview]{
        ...
        \textbf{a,} Models are placed as (empty, purple) nodes on the \emph{active branch} $\mu$, 
            where each model is a sum of terms $\hat{t}_k$ multiplied by corresponding scalar parameters $\alpha_k$. 
        ...
        \textbf{c,} $\mu$ is consolidated, i.e. models are evaluated relative to other
            models on $\mu$, according to the consolidation mechanism specified by the \gls{es}.
...
In summary: 
    ...
    % \gls{qmla} places $\mathbb{H}$ on a unique branch; 
...
At several junctions within \crefrange{alg:qmla}{alg:model_search}, 
...
To solidify the concept of \glspl{es}, and how they affect the overall
...
\begin{easylist}[itemize]
    ...
    && $t_{U(4)} \approx 10^{-3} \textrm{sec}$. 
    & each model undergoes a reasonable training regime;
    && $\Ne=1000, \Np=3000$;
    && $\implies t_{\textrm{QHL}} = \Ne \times \Np \times  t_{U(4)} = 3000s \approx 1 \textrm{h} $;
    & Bayes factor calculations use 
    && $\Ne=500, \Np=3000 $
    && $\implies t_{\textrm{BF}} \approx 2 \times  500 \times 3000 \times  10^{-3} \approx 1 \textrm{h}$;
...
These examples illustrate some of the design decisions involved in composing an \gls{es}, 
...
The approach in (a) is clearly limited in its applicability, 
...
\begin{description}
    ...
    As rigorous as desired by the user. 
    \begin{easylist}[itemize]
...
More precisely, we can view individual models as \emph{learning agents} according to the criteria of 
    ...
    \begin{easylist}[itemize]
        ...
        & a \emph{critic}: the likelihood function informs whether the designed action (experiment) was successful; 
...
\begin{algorithm}
    ...
    \label{alg:qmla}
    \DontPrintSemicolon
    \KwIn{ $Q$ \tcp*[1]{some physically measurable or simulatable quantum system}}
    \KwIn{ $\mathbb{S}$ \tcp*[1]{set of exploration strategies}}
    \KwOut{$\hat{H}^{\prime}$ \tcp*[1]{\gls{champion model}}}\;
    
    $\mathbb{H}_c \gets \{\}$\;   
    \For{$S \in \mathbb{S}$ }{
        $\hat{H}_{S}^{\prime} \gets$ \ttt{model\_search(Q, S)} \tcp*[1]{run model search for this ES, e.g. \cref{alg:model_search}}
...
\begin{algorithm}
    \caption{Exploration strategy subroutine: \ttt{model\_search}}
    ...
    \While{ \ttt{!S.terminate()} }{
        $\mu \gets $ \ttt{S.generate\_models($\nu$)} \tcp*[1]{spawn new models, e.g. \cref{alg:generate_models}}

        \For{ $\hi \in \mu$}{
            $\hi^{\prime} \gets $ \ttt{S.train($\hi$)} \tcp*[1]{train candidate model, e.g. \cref{alg:qhl}}
        }
        $\nu \gets$ \ttt{S.consolidate($\mu$)} \tcp*[1]{consolidate set of models, e.g. pairwise via \cref{alg:consolidate}}
...
\begin{algorithm}
    \caption{Exploration strategy subroutine: \ttt{consolidate} (points per Bayes factor win)}
    \label{alg:consolidate}
    \DontPrintSemicolon
    \KwIn{ $\mu$ \tcp*[1]{information about models considered to date}}
    \KwIn{ $b$ \tcp*[1]{threshold for sufficient evidence that one model is favoured}}
    \KwIn{\ttt{BF()} \tcp*[1]{function to compute the \gls{bf} between $\hat{H}_j$ and $\hat{H}_k$, \cref{alg:bayes_factor} }}    
    \KwOut{ $\hat{H}_{k^{\prime}}$ \tcp*[1]{favoured model within $\mu$}}\;
...
\begin{algorithm}
    \caption{Exploration strategy subroutine: \ttt{generate\_models} (greedy spawn)}
    ...
    $\huc \gets$ \ttt{top\_model}$(\nu)$ \tcp*[1]{find previous branch champion}

    $\{\hat{t}^{\mu}_{c}\} \gets$ \ttt{get\_terms}($\huc$) \tcp*[1]{extract terms of branch champion}

    $\mathcal{T}^{\prime} \gets \mathcal{T} \setminus \{ \hat{t}^{\mu}_{c} \}$ \tcp*[1]{remove terms of branch champion}\;
    $\mathbb{H} \gets \{\}$ 

    \For{ $\hat{t} \in \mathcal{T}^{\prime}$}{
        $\hat{H}_i \gets \huc + \hat{t}$ \tcp*[1]{add one term to the branch champion}
...
\begin{algorithm}
    ...
    \KwIn{$\hat{H}_i$ \tcp*[1]{\gls{hamiltonian} model attempting to reproduce data from $\hat{H}_0$ }}
    \KwIn{$\pra$ \tcp*[1]{probability distribution for $\al = \al_0 $}}
    \KwIn{\gls{nexp} \tcp*[1]{number of \glspl{experiment} to iterate learning procedure for}}
    \KwIn{\gls{nprt} \tcp*[1]{number of \glspl{particle} to draw from $\pra$}}
    \KwIn{\ttt{$\Lambda(\pra)$} \tcp*[1]{\acrlong{edh}}}
    \KwIn{\ttt{RS($\pra, \Np$)} \tcp*[1]{resampling algorithm for redrawing $\Np$ \glspl{particle} from $\Pr(\al)$}}

    \KwOut{$\al^{\prime}$ \tcp*[1]{estimate of \gls{hamiltonian} parameters}}\;

    $\mathcal{P} \gets$ \ttt{RS}$(\pra, \Np)$ \tcp*[1]{sample \glspl{particle} from prior}
    ...
    \For{$ e \in  \{1 \rightarrow \Ne\}$ }{ 
        ...
        $d \gets$ prepare $Q$ in $\ket{\psi}$, evolve and measure after $t$ \tcp*[1]{datum}
        \For{
            $p \in \mathcal{P}$ 
        }{
            $\al_p \gets$ parameter vector corresponding to $p$
            
            $\Pr(d|\al_p; t) \gets \lvert \bra{d} e^{-iH(\al_p)t} \ket{\psi} \rvert^2 $ \tcp*[1]{likelihood}
        ...
        \If{
            ...
            $\mathcal{P} \gets$ \ttt{RS}$(\pra, \Np)$
            \tcp*[1]{redraw \glspl{particle} via resampling algorithm}
            \For{
                $p \in \mathcal{P}$ 
            }{
                $w_p \gets 1/\Np$ \tcp*[1]{set weights for each particle}
    ...
    $\al_i^{\prime} \gets$ \ttt{mean}($\pra) \gets \vec{\alpha}^{\prime}$\;
...
\begin{algorithm}
    ...
    \DontPrintSemicolon
    \KwIn{ $Q$ \tcp*[1]{some physically measurable or simulatable quantum system.}}
    \KwIn{ $\hat{H}_j, \hat{H}_k$ \tcp*[1]{\gls{hamiltonian} models to compare }}
    \KwIn{ $\Pr_{j}(\al), \Pr_{k}(\al)$ \tcp*[1]{posterior distribution following training for $\hat{H}_j, \hat{H}_k$}}    
    \KwIn{$\Np^{\prime}$ \tcp*[1]{number of \glspl{particle} to draw from posteriors for evaluation}}
    \KwIn{ $\expset_j, \expset_k$ \tcp*[1]{\glspl{experiment} on which $\hat{H}_j$ and $\hat{H}_k$ were trained during QHL}}
    
    \KwOut{$B_{jk}$ \tcp*[1]{Bayes factor between two candidate \glspl{hamiltonian}}}\;
          
    $\expset = \{ \expset_j \cup \expset_k \}$ \tcp*[1]{common \glspl{experiment} for fair comparison}\;
    \For{$\hat{H}_i \in  \{\hat{H}_j, \hat{H}_k$ \} }{
        $\tll_{i} = 0$ \tcp*[1]{\acrlong{tltl} for $\hat{H}_i$} 
        \For{$e \in \expset$}{
            $e \gets t, \ket{\psi}$ \tcp*[1]{assign evolution time and \gls{probe} from \gls{experiment} control set}
            $d \gets$ Prepare $Q$ in $\ket{\psi}$, evolve and measure after $t$ \tcp*[1]{datum}\;

            $\mathcal{P} \gets$ \ttt{sample}($\Pr_{i}(\al), \Np^{\prime}$) \tcp*[1]{sample from $\hat{H}_i$'s posterior}

            $\lk_{e} \gets 0$ \tcp*[1]{\acrlong{total likelihood} for $\hat{H}_i$ on this experiment}\;
            \For{$\al_p \in \mathcal{P}$}{
                $ \Pr(d | \hat{H}_i, t) \gets \absval{ \bra{d} e^{-i \hat{H}_i(\al_p) t} \ket{\psi} }^2 $  \tcp*[1]{\gls{likelihood} for particle $\al_p$ on $e$}

                $\lk_{e} \gets \lk_{e} + \Pr(d | \hat{H}_i, t)$ \tcp*[1]{add $l_e$ to \acrlong{total likelihood} }
            }

            $\tll_{i} \gets \tll_{i}+ \ln(l_e) $ \tcp*[1]{add $\ln(l_e)$ to \acrlong{tltl} }
        }
    }
    
    $B_{jk} \gets \exp\left( \tll_{j} - \tll_k \right) $  \tcp*[1]{Bayes factor between models}\;
    return $B_{jk}$
\end{algorithm}
