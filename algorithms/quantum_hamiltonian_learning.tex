...
\begin{figure}
    ...
    \centering
    \subfloat{
        \includegraphics{figure_development/smc_demo/epoch_0.pdf}
    }
    \subfloat{
        \includegraphics{figure_development/smc_demo/epoch_9.pdf}
    }
    \subfloat{
        \includegraphics{figure_development/smc_demo/epoch_10.pdf}
    }
    % \hspace{0mm}
    
    \subfloat{
        \includegraphics{figure_development/smc_demo/epoch_50.pdf}
    }
    \subfloat{ 
    \includegraphics{figure_development/smc_demo/epoch_100.pdf}
    }
    \subfloat{ 
    \includegraphics{figure_development/smc_demo/epoch_500.pdf}
    }
    % \hspace{0mm}
    \subfloat{
        \includegraphics{figure_development/smc_demo/cbar.pdf}
    }
...
\subsection{Interactive Quantum Likelihood Estimation}
...
\begin{figure}
    ...
    \caption[Training with different heuristics]{
        ...
        We show models of various complexity and dimension, each trained using heuristics outlined above.
        \figtableref
    }
...
We show some examples of how the choie of \gls{probe} set can influence the training procedure in \ref{fig:probes_test},
    testing models of various complexity amd dimension. 
In particular, we first a simple 1-qubit model, \cref{eqn:probe_test_1}; 
    followed by more complicated 1-qubit model, \cref{eqn:probe_test_2};
    and randomly generated 5-qubit Ising, \cref{eqn:probe_test_3} and 4-qubit Heisenberg models, \cref{eqn:probe_test_4}.
Each $\hi$ have randomly chosen parameters implicitly assigned to each term. 
\begin{subequations}
    \begin{equation}
        \label{eqn:probe_test_1}
        \h_1=\s_1
    \end{equation}
    \begin{equation}
        \label{eqn:probe_test_2}
        \h_2 = \s_x + \s_y + \s_z
    \end{equation}
    \begin{equation}
        \label{eqn:probe_test_3}
        \h_3 = \sigma_{(1, 3)}^{z} + \sigma_{(1, 4)}^{z} + \sigma_{(1, 5)}^{z} + \sigma_{(2, 4)}^{z} + \sigma_{(2, 5)}^{z} + \sigma_{(3, 4)}^{z} + \sigma_{(3, 5)}^{z}        
    \end{equation}
    \begin{equation}
        \label{eqn:probe_test_4}
        \h_4 = \sigma_{(1, 2)}^{z}+ \sigma_{(1, 3)}^{z}+ \sigma_{(2, 3)}^{xz}+ \sigma_{(2, 4)}^{x}+ \sigma_{(3, 4)}^{z}        
    \end{equation}
\end{subequations}
