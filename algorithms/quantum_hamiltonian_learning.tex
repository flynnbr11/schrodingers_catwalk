...
\begin{figure}
    ...
    \caption[\Glsentryfull{qhl} via \glsentryfull{smc}]{
        \Glsentryfull{qhl} via \glsentryfull{smc}. 
...
In this way, strong particles -- with high $\Pr(d | \al_p; e)$ -- have their weight increased, 
...
\footnotetext{
    Particles are \emph{resampled} according to a resampling algorithm. 
...
The case where the \gls{likelihood} is computed on a quantum simulator is referred to as \gls{qle}
    ...
    \cite{berry2015hamiltonian, childs2018toward}. %TODO references for Hamiltonian dynamics
\par 
...
\footnotetext{
    ...
    of eigenstates orthogonal to $\ket{\psi}$ in \creq{eqn:pr1}.
\par 
...
A fundamental result in \gls{qm} -- the \gls{le} -- undermines the basis of \gls{qle}, 
    which can be overcome by the extension to \gls{iqle} \cite{Wiebe:2014qhl}. 
The \gls{le} shows that the expectation values of two almost identical Hamiltonians 
    diverge afeter exponentially small evolution time, 
    and therefore even hypothetical parameters 
...
For instance, the Hamiltonian for an oscillating electron spin in a \glsentrylong{nvc} 
    is given by
    \begin{equation}
...
\begin{align}
    ...
    \end{split}
\end{align}
Then, if the system measures in $\ket{+}$, we set the datum $d=0$, otherwise $d=1$. 
