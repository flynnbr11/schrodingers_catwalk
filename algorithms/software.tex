...
\begin{figure}[t]
    ...
    \caption[\gls{QMLA} codebase overview]{
        Overview of important \emph{objects} in the \gls{qmla} codebase.
...
The application of \gls{qmla} refers to the choice of target system, and how \gls{qmla} searches the 
...
\footnotetext{Methods are functions which are associated with classes.}
...
The mapping here is trivial: the master of our algorithm is \gls{qmla}, 
    while workers can be used for the training and comparison of models. 
...
We depict the structure of our parallel architecture, and the master-worker strategy, in \cref{fig:parallel}.
...
The infrastructure enabling the distribution of \gls{qmla}'s tasks across a set of worker processes
    can be summarised as a set of classes representing the objects on which we must perform expensive calculations, 
    as well as functions to launch those calculations independently of any other calculation, 
    and a database which can be accessed by \gls{qmla} and all workers.
\par 
...
Crucially, this separation allows us to perform data-heavy calculations independently, 
    ...
    and discard the class instance used for the calculation, 
    while only the relatively small \emph{storage} classs is retained by \gls{qmla} for later use. 
To achieve this separation between calculation and analysis, we use a redis database,
    which holds compressed summaries of the outcomes of tasks. 
