...
Nitrogen vacancies are point defects in diamond, 
    occurring intrinsically (naturally) \cite{davies1976optical} or extrinsically (synthetically) \cite{meijer2005generation, edmonds2012production}.
...
The difference in energy between our defined computational basis states $\ket{0}$ and $\ket{1}$ is 
    $\approx 2.87 \SI{}{\giga\hertz}$, i.e. it is addressable by \gls{mw} radiation.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The spin is again allowed precess freely, here for a duration $t^{\prime}$. 
        Again the $\ket{1}$-component gains a phase $e^{-i \delta t^{\prime}}$, 
...
We will utilise this strategy in \cref{chapter:many_qubits}.
\footnotetext{
...
To relate the experimental procedure to the parameter learning technique described in \cref{chapter:qhl}
    which fulfils the training stage of \gls{qmla}, consider the overall Hahn echo sequence.
...
We vary the evolution time up to $t \approx 4 \mu s$ in intervals of $\Delta t = 50 \SI{}{\nano\second}$, 
...
The hyperfine tensor $\mathbf{A}$ consists of the hyperfine parameters, 
    i.e. the strength of corresponding interactions. 
\begin{equation}
    \mathbf{A} = 
    \begin{pmatrix}
        A_{\perp} & 0 & 0 \\    
...
\begin{figure}
    ...
    \caption[Greedy model search]{
        ...
        Branches are grouped in tiers, corresponding to levels of approximation:
...
So, the first branch is given by $\mu = \{ \hat{S}_x, \hat{S}_y, \hat{S}_z\}$;
    ...
    are not yet considered, so it constructs the models $\{\hat{S}_{x,y}, \hat{S}_{y,z}\}$, e.g. \cref{fig:greedy_search}. 
...
This champion reducibility test simply removes the negligible-parameter terms from $\hp$, 
...
Even so, the remaining $2^9$ permitted models were reduced further by building the logic of this \gls{es} 
    from our intuition around existing knowledge of typical \gls{nvc} systems.
As such, the described \gls{es} will only ever consider $<20$ models per instance. 
The described \gls{es} seems overly prescriptive, 
...
As discussed in \cref{sec:heuristic}, usually it is helpful to allow an \gls{edh} to
    choose the experimental controls, including the evolution time, $t$, against which 
...
We first focus on the overall outcomes:
    ...
    overfits or underfits the true parameterisation. 
...
\begin{figure}
    ...
    \caption[QMLA applied to experimental nitrogen-vacancy centre system]{
        ...
        The \emph{mis-parameterised} class groups models with the same parameterisation cardinality as $\ho$, but different \gls{hamiltonian} terms. 
...
Then, considering the performance of the algorithm on whole, 
    we perform \glspl{run} of 100 \glspl{instance} on the experimental data as well as simulated data,
...
\begin{figure}
    ...
    i.e. models which may reasonably be expected to describe the targeted \glsxtrlong+{nvc} from theoretical arguments.
...
The key insight promised by \gls{qmla} is to identify the interactions present in the studied system, 
...
Conversely, in our simplistic two-qubit model, the spectrum of eigenenergies is artificially restricted, 
...
Invoking a finite-size bath to approximate the genuine open-system nature of the \gls{nvc} 
    ...
    sufficient to characterise the dominant decoherence processes of a system, as we have done in this chapter.
...
In \cref{chapter:many_qubits} we consider a similar \gls{nvc} setup in simulation, 
    ...
    and we note in \cref{chapter:outlook} that in order for \gls{qmla} to  meaningfully characterise
    genuine quantum systems, it must explore models of open systems. 
