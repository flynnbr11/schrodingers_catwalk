...
Of the \gls{nitrogen} atom's five valence electrons, three bond with nearby \glspl{carbon};
    the remaining two unbonded electrons form a lone pair and can be thought of as a single spin-$\frac{1}{2}$ particle. 
...
The single electron in the lattice vacancy, together with the effective lone pair of the \gls{nitrogen}, 
    form a system of two spin-$\frac{1}{2}$ particles: such systems have been thoroughly studied.
...
\begin{figure}[t]
    ...
    \caption[\Glsentrylong{nvc} energy levels.]{
        ...
        \textbf{c},  Application of a \SI{532}{\nano\metre} laser (green arrow) excites the \glsentrylong{nvc} from any of the states in the 
            ground state manifold into the excited manifold. 
        The dominant decay mechanism for the excited states are shown: 
            (i) $\ket{m_s=0}_e \rightarrow \ket{m_s=0}_g$ (\glsentrylong{pl}, red) through the emission of a photon at \SI{637}{\nano\metre});
...
We can then apply \gls{mw} radiation to achieve quantum logical gates on the system, 
    ...
    and observing the \glsentrylong{pl} (i.e. the emitted photons), and inferring the population level of each basis state. 
...
\footnotetext{
    ...
    classical exponentiation of the $2^{n_q} \times 2^{n_q}$ Hamiltonian for each particle and experiment in \gls{cle}, 
...
\begin{figure}
    ...
    \caption[States of spin qubit at each stage of Hahn echo sequence]{
        ...
        \textbf{(b)} We apply a rotation about the $y$-axis (i.e. a \glsentryfull{mw} pulse), 
...
By varying the evolution time of the Hahn echo sequence, we can map the \gls{likelihood} 
    against time, which we can view as capturing the dynamics of the \gls{nvc} spin \cref{fig:nv_raw_data}.
...
\begin{figure}[t]
    ...
    \caption[Raw data for \glsentrylong{nvc}'s dynamics]{
        ...
        The $y$-axis shows the normalised \glsentrylong{pl} of the \gls{nvc}, 
...
\begin{figure}
    ...
    \caption[\gls{qmla} applied to experimental \glsentrylong{nvc} system]{
        \gls{qmla} results on simulated and experimental data, describing a \glsentryfull{nvc} system. 
        \textbf{a}, 
        ...
        Simulated \glspl{likelihood}  reproduced by the model with the highest  \gls{win rate}  ($\hat{S}_{x,y,z}\hat{A}_{z}$, turquoise), 
            ...
            (red dots, extracted from the observed \glsentrylong{pl} of the first Hahn echo decay). 
        ...
        \textbf{f}, 
        A single QMLA \gls{instance} against experimental data in \textbf{(e)}, depicted as a \glsentrylong{dag}.
...
Comparisons between models, $\hi, \hj$,  are shown as edges between nodes on the graph, 
    coloured by the strength of evidence of the outcome, i.e. the \glsentrylong{bf}, $\bij$. 
...
\begin{figure}
    ...
    \end{center}
    \caption[
        Models considered by \gls{qmla} for simulated/experimental \glsentrylong{nvc} data, and their  \glspl{win rate}
    ]{
    ...
    i.e. models which may reasonbaly be expected to describe the targeted \glsentryfull{nvc} from theoretical arguments.
