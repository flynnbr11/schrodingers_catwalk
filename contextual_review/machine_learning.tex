...
\begin{figure}
    ...
    \caption[Confusion matrix]{
        ...
        \glsentrylong{tp}, \glsentrylong{tn}, \glsentrylong{fp} and \glsentrylong{fn}.
    }
...
\begin{table}
    ...
    \end{tabular}
    \caption[$\fs$ examples]{
        Examples of how \fs behaves for varying \glsentrylong{tp}, \glsentrylong{fn} and \glsentrylong{fp}. 
    }
...
A growing domain is the development of \gls{ml} algorithms which run on quantum hardware, 
    or exploit data from quantum systems; generally referred to as \glsentryfull{qml} \cite{biamonte2017quantum, adcock2015advances}. 
...
In later chapters (\cref{chatper:ga} and \cref{chapter:many_qubits}) 
...
\begin{algorithm}
    ...
    \For{$i \in 1, ..., N_g$}{
        ...
        \While{$| \mu | < N_m$}{
            ...
            $c_1, c_2 \gets \ttt{mutate}(c_1,c_2)$ \tcp*[1]{possibly mutate offspring}
...
The fitness of any candidate is then given by the total value of that configuration of objects, $v_j = \vec{v} \cdot \vec{\gamma}_j$, 
    ...
    is less than the capacity of the knapsack, i.e. $w_j = \vec{w} \cdot \vec{\gamma}_j \leq w_{max}$. 
\footnotetext{
...
Considerations are hence usually built into the \glsentrylong{of};
    \gls{ga} implementations later in this thesis therefore demand we design \glsentrylongpl{of} 
