...
We also have the \emph{dual vector} of \cref{eqn:unitary_time_evolution},
\begin{equation}
    \label{eqn:dual_unitary_evolution}
    \bra{\psi(t)} = \bra{\psi(t_o)}\hat{U}^{\adjoint},
\end{equation}
    see \cref{sec:linear_algebra}. 
Combining \crefrange{eqn:conservation_simularity}{eqn:dual_unitary_evolution}{eqn:unitary_time_evolution}, 
\begin{equation}
    \begin{split}
...
So far there is no restriction on $\ho$\footnotemark.
    \footnotetext{
...
Recalling the unitarity condition, and that $\lim_{\epsilon \rightarrow 0} \mathcal{O}(\epsilon^2) \sim 0$, 
    we have:
\begin{equation}
...
In the previous section we introduced the Hamiltonian of \gls{q} as the generator of its 
...
We saw in \cref{eqn:hamiltonian_hermiticity} that $\ho$ is Hermitian;
    the Hamiltonian is the observable operator (see \Crefrange{postulate:eigenvalues}{postulate:hermiticity} in \cref{sec:postulates}) 
...
In general\footnotemark, a state can be seen as a \emph{superposition} across the system's eigenstates, $\left\{\ket{v_i}\right\}$.  
...
\footnotetext{
    The terms \emph{eigenstate}, \emph{eigenfunction} and \emph{eigenvector} are interchangeable.
}
...
In other words, if we prepared $\ket{\psi}$ and measured it -- via some observable, $\hat{A}$ -- 
    ...
    then as $N \rightarrow \infty$, the average outcome is the \emph{\gls{expecatation value}}
    for the system.
\begin{equation}
...
The initial motivation for the development of quantum technologies, however, 
    ...
    which can accurately emulate the true dynamics \cite{feynman1982simulating, manin1980vychislimoe, benioff1980computer, benioff1982quantum}. 
