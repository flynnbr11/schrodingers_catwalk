...
\footnotetext{
    ...
    profitably use knowledge of Hamiltonians on a quantum computer. 
}
...
Upon measurement, the state vector of $Q$ has amplitude associated with only one eigenvector. 
...
In constructing quantum computers and dealing with their output, 
...
Furthermore, there are a number of criteria required before a quantum computer can be deemed functional \cite{divincenzo2000physical}. 
\par 
...
Any two-level quantum system can be used as a qubit;
    ...
    among the most popular\footnote{listed with their primary advantages and limitations}:

\begin{easylist}[itemize]
    & Photonic qubits (linear optical quantum computers) \cite{kok2007linear}
...
Our restriction to classical resrouces leads to a few remarks:
\begin{easylist}[itemize]
    & given access to a fault-tolerant quantum computer/simulator,
    ...
    && it is believed that the same calculation can be performed in polynomial time on a quantum computer \cite{lloyd1996universal}. 
