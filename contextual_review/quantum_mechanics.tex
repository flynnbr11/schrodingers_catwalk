...
It seems, then, that quantum computers are beginning to represent a viable information processing tool. 
In this thesis, we do not run algorithms directly on such devices, 
    but instead simulate the outcomes on classical hardware:
\begin{easylist}[itemize]
    & the results achieved are limited by the capability of classical computers in simulating quantum systems
    ...
    && the remit of this thesis is therefore to robustly test the presented algorithms,
    ...
    && it is believed that the same calculation can be performed in polynomial time on a quantum computer. 
\end{easylist}
