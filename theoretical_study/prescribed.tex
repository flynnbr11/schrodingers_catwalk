...
\footnotetext{
    Including \cref{sec:family_identification} where we study the effect of varying resources on \gls{qmla}'s outcome
...
\section{Model Family Identification}\label{sec:family_identification}
\glsreset{es}
\glsreset{et}
Recall from \cref{sec:qmla_protocol} (and \cref{fig:qmla_overview}\textbf{(e)}) that \gls{qmla} 
    is built to grow multiple \glspl{et} concurrently, each corresponding to a unique \gls{es}. 
...
For instance, we can run \glspl{es} for each of the Ising, Heisenberg and Hubbard models, 
    and determine which formalism best describes the system of interest, \gls{q}. 
\par 

Earlier in this chapter, we alluded to a fundamental question in and any discussion of model training/comparison 
    through Bayesian inference as underlies \gls{qhl}:
    to what extent do training resources ($\Ne, \Np$) undermine trained models, 
    and what resources should be granted in order to retrieve reliable outcomes?
As usual in \gls{ml} methods, this represents a trade-off between the results of the algorithm with training time
    and computational resources required.
\par

We combine non-exhaustive testing of the tradeoff between resources and outcomes, 
    with the task of identifying the family of models to which \gls{q} belongs. 
...
\cref{fig:family_identification} shows the rate at which the precise $\ho$ is identified, 
    as well as the rate with which family of $\ho$ is identified, 
    compared with increasing resources $\Ne, \Np$. 
A clear trend demonstrates that success scales with resources.
...
\gls{qmla}'s capacity to identify the family of model to which \gls{q} belongs suggests powerful 
    future applications of the framework, namely to automatically discover the type of physics underlying 
    systems of interest.
For example, \gls{qmla} could be used to identify whether a sample is ferromagnetic or antiferromagnetic;
