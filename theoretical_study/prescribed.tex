...
Conversely, miscalibrations can mainfest as imperfectly implemented gates/steps in the calculation of the system's \gls{likelihood}, 
...
The \gls{es} in this chapter is then simply to propose a set of models $\mathbb{H}$,
    ...
    and comparisons between all pairs of models through \glspl{bf}.
\par
...
This serves as a good \emph{sanity check}, confirming our expectation that 
    ...
    are trained to a high precision to very similar parameters, and are difficult to distinguish through human intuition. 
...
We will consider the fully parameterised model in \cref{chapter:ga};
    ...
    suc that the parameters and terms of interest are then captured by \cref{eqn:heisenberg_terms}.
...
We can then devise a simple \acrfull+{es} which only tests the models corresponding to lattices in $\mathbb{C}$, 
