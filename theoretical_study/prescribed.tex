...
and solving combinatorial optimisation algorithms, namely \ttt{MAX-CUT}, 
...
\end{center}
\caption[\Glsentrytext{qhl} for the fully parameterised Ising model]{
...
\footnotetext{When the model being trained is $\ho$, the volume may not saturate until numerical precision is reached \cite{Wiebe:2014qhl}.}
...
\footnotetext{
Here we have deliberately described the special case where models of more- and fewer-parameters are identical, 
and the fewer-parameter-model can exactly reproduce the larger parameterisation. 
Had the true model been composed of unequal $\{J_{kl}\}$, the under-parameterised model should not be preferred.
}
...
\KwOut{$\{ \hi \}$ \tcp*[1]{set of models to test}}\;
...
\For{$\hi \in \mathbb{H}$}{
    \For{$\hj \in \mathbb{H}\setminus \{\hi\}$}{
        ...
        \If{$\bij > 1$}{
            $s_i \gets s_i + 1$ \tcp*[1]{$\hi$'s score increases if it is favoured by the \gls{bf}}
        }
    }
}
...
For example, we adopt the fully connected four site lattice (\cref{fig:lattices}\textbf{(d)})
...
\footnotetext{
The limitation of Hubbard models to $4$-site lattices is due to the $6$-qubit models required via the \gls{jwt}.
...
\footnotetext{
Recall the main resources for model training: 
the number of \glspl{experiment} performed, \gls{nexp}, 
and the number of \glspl{particle} used during \gls{qhl}, \gls{nprt}.
}
\par
