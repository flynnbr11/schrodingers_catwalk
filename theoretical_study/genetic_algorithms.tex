The \gls{qmla} framework lends itself easily to the family of optimsation techniques called \emph{evolutionary algorithms}, 
    where individuals, sampled from a population of candidates, are considered as solutions to the given problem in generations,
...
This is a tenet of \gls{qmla}: for each candidate $\hj ( \al_j )$, we use a subroutine to optimise $\al_j$\footnotemark.
\footnotetext{As in earlier applications of \gls{qmla}, for this study we rely on \gls{qhl} as the parameter optimisation subroutine.} 
\par
...
\begin{easylist}[itemize]
    ...
    & new models are spawned through the \gls{ga} by selecting pairs of parents for crossover, 
...
$F_{\beta}$-score (\cref{eqn:f_beta}) is a measure which balances these, with $F_1$-score in particular giving them equal importance. 
\begin{equation}
    \label{eqn:f1_score}
    F_1 = \frac{2\times (\textrm{precision})\times(\textrm{sensitivity})}{(\textrm{precision} + \textrm{sensitivity})} = \frac{TP}{TP + \frac{1}{2}(FP + FN)} =: f.
\end{equation}
...
Finally, in addition to reducing the resources used per \gls{bf} calculation, 
    ...
    as would be required for rating models according to the \gls{of} to be described in \cref{sec:elo}, 
...
\end{easylist}
\footnotetext{  
    These and further \glspl{hyperparameter} can be swept using code given in the \gls{qmla} codebase, 
...
We must also consider, however, that -- in realistic cases of \gls{qmla} -- the total computation time scales badly with these parameters, 
...
\begin{table}    
    ...
    \caption[Objective function examples]{
        ...
        For each model we first summarise its
            average  \gls{likelihood} $\lk_{e}$ (Eqn. \ref{eqn:log_likelihood}),     
...
At each generation, all models -- including the elite models from the prvious generation -- 
are assigned the same initial rating. 
This ensures that we do not unfairly favour elite models: 
    new models have a reasonably chance of overtaking the elite models.
...
\begin{figure}
    ...
    \end{center}
    \caption[Single model within a single generation of \gls{qmla} \glsentrylong{ga}]{
...
Consequently, even in cases where the precise model, $\ho$, is not identified, the \gls{champion model}
