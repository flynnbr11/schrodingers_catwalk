...
In \cref{fig:bf_by_fscore}b, we run a complete model training subroutine, but compute the \gls{bf} based on fewer experiments and \glspl{particle} 
...
\begin{figure}
    ...
    \caption[Bayes factors by $\fs$]{
        ...
        \textbf{b}, $\Ne=500, \Np=2500$ using only a fraction (0.2) of experiments/\glspl{particle} for \gls{bf} calculations. 
        ...
        \textbf{d}, $\Ne=500, \Np=2500$, comparing only a subset of pairs of models through \glspl{bf}, 
            and using only a fraction ($0.2$) of experiments/\glspl{particle} for those calculations. 
...
\begin{easylist}[itemize]\label{list:obj_fnc_terms}
    ...
    & $\expset_i$, the bespoke set of experiments composed by the \acrlong{edh} (\cref{sec:heuristic}) solely for trainng $\hi$;
...
$\tll_{i}$, defined in \cref{eqn:log_total_likelihood}, can be thought of as a measure of the 
    ability of a given model to reproduce a given dataset $expdata$ from experiments $\expset$.     
% success of a given model at explaining data from any set of experiments, $\expset$. 
...
This allows us to calculate the \emph{residual} between the system and individual \glspl{particle}' \glspl{likelihood}, $r_p^e$; 
    we can hence compute the mean residual across all \glspl{particle} in a single experiment $r^e$:
\begin{align}
