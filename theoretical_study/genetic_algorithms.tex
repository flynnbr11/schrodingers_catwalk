...
Note that the time saving reported between full and partial connectivity between models scales with $N_m$: 
    ...
    for $N_m=28$, these rise to 190 and 378 respectively. 
...
For practical reasons\footnotemark, we set $N_m=28$ and $N_g=32$, although in most cases the 
...
\begin{figure}
    ...
    \end{center}
    \caption[Single model within a single generation of the \gls{qmla} \gls{ga}.]{
        \glsentryfull{bfeer} progression for a single candidate, $\hi$, within a single generation.
...
The gene pool as a whole tends towards a homogeneous set of models, all with $f \gapprox 0.9$. 
...
\begin{figure}
    ...
    \caption[\Gls{run} of \gls{qmla} \gls{ga}.]{
        A \gls{run} of the \gls{qmla} \gls{ga}, consisting of 50 independent \glspl{instance}.
        ...
        \textbf{(c)}, Champion models from each instance, showing \gls{qmla} finds strong models 
            in general, and in particular finds the true model $\ho$ (with $f=1$) in $32\%$ of cases. 
...
Finally, to understand the performance of the \gls{qmla} algorithm overall, 
    we run 50 independent instances in a \gls{run}, \cref{fig:ga_run}. 
We see that, while the overall model space can be characterised by the distribution 
    of models' $\fs$, where we sample where $\overbar{f} = 0.5 \pm 0.13$, 
    that \gls{qmla} quickly moves to the subspace of high-quality models, 
    and ultimately nominates champion models, $\{ \hp \}$ with $f > 0.9$ in \todo{fill in final values}, 
    and precisely identifies $\hp=\ho$ in \todo{fill in final values}$\%$ of instances. 
