...
Candidates are batched in \emph{generations},
    ...
    by mimicking biological evolutionary mechanisms \cite{back1996evolutionary}. 
...
\begin{easylist}[enumerate]
    ...
    & Sample $N_m$ models from the total population, $\population$, at random
...
\end{easylist}
\footnotetext{
    The designation of \gls{qmla} as supervised, unsupervised, or otherwise depends upon the \gls{es} employed, \cref{sec:agency}. 
    The model search of this \gls{ges} is unsupervised, but we use metrics from literature about classification, 
        since we can assess performance absolutely.
}
...
Of course in realistic cases we can not assume knowledge of $\termset_o$ and therefore cannot compute 
    \gls{fscore}, but it is a useful tool in the development of the \gls{ges} itself, 
    or in cases where $\ho$ is known, such as when the target system is simulated, e.g. in the case of device calibration.
...
In \cref{fig:bf_by_fscore}, we show the relationships between \gls{fscore} and \gls{bf} for various conditions. 
...
Some comparisons near the diagonals of \cref{fig:bf_by_fscore} favour the model with lower \gls{fscore}, 
...
On the other extreme, in \cref{fig:bf_by_fscore}\textbf{(c)}, where models are trained with
...
Firstly we will validate our reasoning that \gls{fscore} is a sensible figure of merit, 
...
We can then sweep over the \gls{ga} \glspl{hyperparameter} to find a suitable configuration:
    in \cref{fig:ga_param_sweep} we show how the choice of parameters affect the success rate of precisely identifying the 
...
\begin{easylist}[enumerate]
    ...
    & number of models per generation, $N_m$;
    & mutation rate, $r_m$;
...
We see that, unsurprisingly, the \gls{ga} performs poorly when run with few resources, 
    but broadly the performances are similar provided it is run with sufficient resources.
...
\begin{easylist}[itemize]\label{list:obj_fnc_terms}
    ...
    & $\expset_i$, the bespoke set of \glspl{experiment} composed by the \acrlong{edh} (\cref{sec:heuristic}) solely for training $\hi$;
    & $n = |\expset_i|$, the number of samples (datapoints) used in training $\hi$. 
\end{easylist}
...
Its primary flaw, however, is its reliance on $\expset_v$: 
    ...
    the design of which can not be guaranteed in advance, or at least risks introducing strong bias
    towards some models. 
...
Note that this is computationally expensive: 
    ...
    where each \gls{particle} corresponds to a unitary evolution and therefore the calculation of a matrix exponential. 
...
\cref{eqn:ranking} has a similar effect to \cref{eqn:bf_points} but awards higher selection probability to 
...
\footnotetext{For consistency with QInfer \cite{qinfer-1_0} -- on which \gls{qmla}'s code base builds --
    we call the \gls{expectation value} for the system $\Pr_Q(0)$;
...
$g_i^r$ encapsulates how well the candidate model reproduces a particular set of dynamics from the target system, 
...
This \gls{of} provides a useful test for \gls{qmla}'s \gls{ga}:
    ...
    mimic $\ho$, then this \gls{of} guarantees to promote  the strongest models,
...
There is a direct parallel between these types of competitions and \gls{qmla}:
    we similarly have a pool of individual competitors (models), 
    which we can place in direct competition, 
    and quantify the comparative outcome through \gls{bf}, 
    in order to determine the preferred candidate. 

\par 
\glspl{Elo rating} are transitive: given some inter-connectivity in a generation, 
...
While every pair of models $(\hi, \hat{H}_{k})$ are not directly connected, 
    there is always a chain of length $l \leq l_{\rm{max}}$ edges between them.
For $N_m=60$, we find $l_{\rm{max}}=2$, 
    e.g. for $\hi, \hat{H}_k$ disconnected, there are comparisons $\{(\hi, \hj), (\hj \hat{H}_k)\}$. 
    
\par 

The \gls{Elo rating} scheme is a nonlinear points transfer system, as follows: 
...
As such, this is a fairer mechanism than \gls{bf} points, 
    ...
    then $\Delta R_i$ changes the credence of $\hi$ proportional to the latest evidence. 
It achieves this by first computing the \emph{expected} result of a given comparison
    with respect to each model, with an exponential response to the difference in their current ratings, 
...
\begin{table}
    ...
    \caption[Example of Elo rating updates]{
        ...
        We demonstrate the effect when there is strong evidence\footnotemark \ 
            in favour of either model through \gls{bf} comparison, $\bij \approx \mathcal{O}(10^{100})$. 
...
This poses the strong possibility for a form of generational wealth:
    ...
    their \glspl{Elo rating} will be higher than all alternatives by definition. 
...
\begin{figure}[t]
    ...
    \caption[Comparison between proposed objective functions]{
        ...
        Each \gls{of} trains the same initial generation of $N_m=28$ models with resources
            $N_E=500, N_P=2000$, and then design a new set of $N_m$ models, $\mu_{\rm{h}}$,        
            through a roulette strategy, such that the only difference between \gls{of}'s output 
        ...
        \textbf{a}, Box-plot of \gls{fscore}, $f$, for the models in $\mu_{\rm{h}}$ in each case,
        ...
        \textbf{c}, Difference between the median $f$ among the 
...
This underlines the risk in building $\expset_v$, which can be biased towards weak models, 
...
We train the same batch of $N_m=28$ random models in each case, and allow each \gls{of} 
    ...
    and therefore direct the design of the hypothetical next generation of models, $\mu_{\rm{h}}$. 
\cref{fig:obj_fnc_comparison} shows the distribution of \gls{fscore} for the models within $\mu_{\rm{h}}$, 
    as generated by each proposed \gls{of}, 
    also accounting for the time taken in each case, 
    i.e. we report the time to train and evaluate the single generation on a 16--core node.
\par
...
In particular, in \cref{chapter:many_qubits}, we will later use $g^r_i$ 
    -- the residual \gls{of} described in \cref{sec:residuals} -- 
    to study a larger \gls{model space} under assumptions of perfect parameter learning.
...
\begin{easylist}[itemize]
    ...
    & The top two elite models from $\mu$, $ \hat{H}_e^{\mu}$, are included on the subsequent generation $\mu+1$.
...
\footnotetext{
    ...
    This restricts the present analysis to simulatable, rather than physical, use cases, 
    e.g. device calibration. 
}

We will use a four-qubit \gls{model space} under the fully parameterised Heisenberg formalism, \cref{eqn:heisenberg_model_general}, 
...
\footnotetext{
    This is to ensure, with 15 available worker nodes,
...
The true parameters $\al_0$ are assigned randomly in the range $(0.25, 0.75)$; 
    within \gls{qhl} the prior is set as a multivariate normal distribution with 
    mean and standard deviation $0.5 \pm 0.125$. 
...
\begin{figure}
    ...
    \caption[Ratings of all models in a single genetic algorithm generation]{
        ...
        Only $\tau=\nicefrac{1}{3}$ of models are assigned selection probability, 
            meaning only those models depicted in the pie chart can become parents to new candidates, 
...
\begin{figure}
    ...
    \caption[Run of QMLA genetic algorithm]{
        ...
        \textbf{b}, The models explored during the model search of all \glspl{instance} combined, $\{\hat{H}_i\}$.
            \gls{qmla} generates $\mathcal{O}(43,000)$ chromosomes in total across the 100 instances,
                i.e. each \gls{instance} trains $\mathcal{O}(430)$ distinct models;
                Models generated by \gls{qmla} are described overall by a distribution of $f = 0.76 \pm 0.15$, 
        \textbf{c}, Champion models from each instance, showing in general \gls{qmla} nominates champion models with
...
Firstly, the set of conceivable terms must be prescribed in advance to facilitate a chromosome mapping;
    ...
    or requires a vast set of permissible terms, leading to substantially larger \gls{model search} phases than shown here. 
